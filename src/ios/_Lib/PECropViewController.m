...
- (void)viewDidLoad
{
    ...
    if (!self.toolbarItems) {
        ...
        UIBarButtonItem *constrainButton = [[UIBarButtonItem alloc] initWithTitle:PELocalizedString(@"Constrain", nil)
               ...
               UIBarButtonItem *rotateButton = [[UIBarButtonItem alloc] initWithTitle:PELocalizedString(@"Rotate", nil)
                                                                             style:UIBarButtonItemStyleBordered                                                                            target:self
                                                                            action:@selector(rotate:)];
        self.toolbarItems = @[flexibleSpace, constrainButton, flexibleSpace, rotateButton];
...
{
    CGFloat radiants = self.cropView.rotationAngle;
    NSLog(@"%f", radiants);
    
    if(self.cropView.rotationAngle <= (M_PI * 270.0f)/180.0f )
    {
        [self.cropView setRotationAngle:radiants + (M_PI * 90.0f)/180.0f];
        //self.cropView.rotationAngle =
    }
    else
    {
        [self.cropView setRotationAngle:1.0f];
    }
    
    //UImage img_test = self.cropView.image.;
    //[img_test hei];
